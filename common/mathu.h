               ...
               Eigen::Transform<S2, 3, Eigen::Affine> &poseA3)
{
  ...
  // cos(yaw)*cos(roll)+sin(yaw)*sin(pitch)*sin(roll)
  // sin(yaw)*sin(pitch)*cos(roll)-cos(yaw)*sin(roll) -sin(pitch)
  // cos(pitch)*sin(roll)                             cos(pitch)*cos(roll)

  // poseA3(0, 0) = A * C; poseA3(0, 1) = A * DF - B * E; poseA3(0, 2) = B * F +
  // A * DE; poseA3(0, 3) = poseV6[0];
  //
  // poseA3(1, 0) = B * C; poseA3(1, 1) = A * E + B * DF; poseA3(1, 2) = B * DE
  // - A * F; poseA3(1, 3) = poseV6[1];
  //
  // poseA3(2, 0) = -D;    poseA3(2, 1) = C * F;          poseA3(2, 2) = C * E;
  // poseA3(2, 3) = poseV6[2];
  //
  // poseA3(3, 0) = 0;     poseA3(3, 1) = 0;              poseA3(3, 2) = 0;
  // poseA3(3, 3) = 1;

  auto A = cos(poseV6[5]), B = sin(poseV6[5]), C = cos(poseV6[4]),
       D = sin(poseV6[4]), E = cos(poseV6[3]), F = sin(poseV6[3]), DE = D * E,
       DF = D * F;
...
Qleft(const Eigen::QuaternionBase<Derived> &q)
{
  ...
  ans.template block<3, 1>(1, 0) = qq.vec(), // (1, 0)???
      ans.template block<3, 3>(1, 1) =
          qq.w() * Eigen::Matrix<typename Derived::Scalar, 3, 3>::Identity() + skew(qq.vec());
  return ans;
  /****************
  ans =
  w    -q0     -q1    -q2
